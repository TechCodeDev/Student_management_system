...
if($found==1)
{
    ...
    $c_name = $_REQUEST['firstname'];
        ...
        $p_radr = $_REQUEST['gmail'];
        $p_addr=$_REQUEST["radress"];
                   
                   
   global $buffer;
   global $c_application, $c_name,$c_lname,$c_dfb, $p_phno,$p_radr, $p_cate,$p_addr,$sound,$ftp,$patterns;
   $buffer=$c_application."|".$c_name."|".$c_lname."|".$c_dfb."|".$p_phno."|". $p_cate."|".$p_radr."|". $p_addr."|"."#".PHP_EOL;

   
   $ftp=fopen("form4.txt","r");
        while(! feof($ftp))
        {
	        $raj=fgets($ftp);
